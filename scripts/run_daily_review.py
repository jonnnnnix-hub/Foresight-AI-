...
def main() -> None:
    ...
    if summary_path.name:
        specialist_rows = "\n".join(
            f"| {sr.specialist_name} | {sr.health_score:.0f}/100 | {sr.severity.value} |"
            for sr in review.specialist_reviews
        )
        summary_path.write_text(
            f"## Council Review — {review_date}\n\n"
            f"**Status:** {review.status.value.upper()}  |  "
            f"**Health:** {review.overall_health:.0f}/100\n\n"
            f"| Specialist | Score | Severity |\n"
            f"|------------|-------|----------|\n"
            f"{specialist_rows}\n\n"
            f"**Trades:** {review.cumulative_trades}  |  "
            f"**Win Rate:** {review.cumulative_wr:.1%}  |  "
            f"**P&L:** ${review.cumulative_pnl:+,.0f}\n",
        )
